class pwnyonlinestore::install {
  ...
  $docroot = '/var/www/pwnyonlinestore'
  $db_username = 'pwny'
  ...
  file { $docroot:
    ...
    owner => 'www-data',
    group => 'www-data',
    source => 'puppet:///modules/pwnyonlinestore/www-data',
  ...
  file { "$docroot/mysql.php":
    ...
    owner => 'www-data',
    group => 'www-data',
    content => template('pwnyonlinestore/mysql.php.erb'),
    notify => File["/tmp/pwny.sql"],
  }
  ...
  file { "/tmp/pwny.sql":
    ...
    content => template('pwnyonlinestore/pwny.sql.erb'),
  ...
  file { "/tmp/mysql_setup.sh":
    ...
    source => 'puppet:///modules/pwnyonlinestore/mysql_setup.sh',
