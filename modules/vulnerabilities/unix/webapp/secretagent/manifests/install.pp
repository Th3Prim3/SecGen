class secretagent::install {
  ...
  file { $docroot:
    ...
    owner => 'www-data',
    group => 'www-data',
    source => 'puppet:///modules/secretagent/www-data',
  ...
  file { "$docroot/mysql.php":
    ...
    owner => 'www-data',
    group => 'www-data',
    content => template('secretagent/mysql.php.erb'),
    notify => File["/tmp/secretagent.sql"],
  }

  # Database Setup
  file { "/tmp/secretagent.sql":
    owner  => root,
    group  => root,
    mode   => '0600',
    ensure => file,
    content => template('secretagent/secretagent.sql.erb'),
  ...
  file { "/tmp/mysql_setup.sh":
    ...
    source => 'puppet:///modules/secretagent/mysql_setup.sh',
